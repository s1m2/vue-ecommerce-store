      ...
      <div class="grid lg:grid-cols-[1fr_4fr] gap-10">
        ...
        <div v-else>
          <div class="flex justify-between">
            <button class="flex gap-3 place-items-center mb-6 border py-2 px-4 rounded-3xl" v-if="selectedCategory">{{ selectedCategory }} <IconClose class="pointer" height="18" width="18"/></button>
            <button class="flex gap-3 place-items-center mb-6 border py-2 px-4 rounded-3xl">Sort by: Highest Price </button>
          </div>
          <div class="grid grid-cols-auto-fit gap-4">
            <Product v-for="(product, index) in allProducts" :key="index" v-bind="product" />
          </div>
        </div>
      </div>
    </div>
